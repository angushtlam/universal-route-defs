...
const {route, routes} = require('../index')
...
describe('route retrieval', () => {
  describe('should properly get top level route', () => {
    const urls = routes({
      ...
      referral: route(
        (props = {referralId: ':referralId'}) => `/referral/${props.referralId}`
      ),
    })
  ...
  describe('should properly get nested route', () => {
    const urls = routes({
      shop: route(() => '/shop', {
        ...
        item: route((props = {itemId: ':itemId'}) => `/item/${props.itemId}`, {
          preview: route(() => '/preview'),
          variant: route(
            (props = {variantId: ':variantId'}) => `/variant/${props.variantId}`
          ),
        }),
      }),
    })
    ...
    it('with no props provided', () => {
      ...
      assert.deepEqual(
        urls.shop.coupons.active.view.get(),
        '/shop/coupons/active/view'
      )
    })
